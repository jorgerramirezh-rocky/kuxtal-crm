...
vm.runInContext([
  ...
  "var ABIERTO={}, SOCIO={}, ENVIANDO=new Set(), TOPE_INTENTOS=3;",
  linea('const estDe='),
  ...['partesGT', 'tsGT', 'hoyStr', 'fechaCorta', 'btnRes', 'funCardHTML', 'pasa'].map(extraer),
].join('\n'), ctx)
...
ok(h.includes("anotar(7,'no_contesta')"), '«No contestó» anota directo (un toque)')
...
h = ctx.funCardHTML(base)
ok(h.includes('vuelve a mí a esa hora'), '«Reprogramar» abre día + hora')
ctx.ABIERTO[7] = null
