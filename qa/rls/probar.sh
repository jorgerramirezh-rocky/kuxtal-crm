...
REP9="$SOLO3 insert into funnel_bases(id,nombre,total) overriding system value values (900301,'b9',9); insert into funnel_prospectos(nombre,base_id) select 'r'||g,900301 from generate_series(1,9) g;"
caso "reparto parejo: 9 entre 3 → 3/3/3" "3,3,3" gerente_tmk "$REP9" "select funnel_repartir(900301); $DUE select string_agg(n::text,',') from (select count(*) n from funnel_prospectos where base_id=900301 group by tmk_id) s;"
