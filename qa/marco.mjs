...
console.log('\n(d) montoMembresia + registrarEnganche (api stub)');
{
  ...
  const calls=[];
  const apiStub=async(path,opt)=>{ calls.push({path,opt}); return {ok:true,json:async()=>[{id:55}]}; };
  ...
  // Bloque 8: la RLS filtra la fila → PATCH «ok» con 0 filas. Eso NO es guardado (antes se perdía en silencio).
  const stub0=async(path,opt)=>({ok:true,json:async()=>[]});
  const r0=await sandbox.registrarEnganche(55,9,250,'12 cuotas',stub0,true,'qa');
  ok(!r0.patched, 'PATCH que no tocó ninguna fila → patched=false (no se da por guardado)');
  calls.length=0;
